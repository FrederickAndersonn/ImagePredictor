    ...
    while True:
        for i in indices:
            ...
            if len(images) >= batch_size:
                yield np.array(images), {'age_output': np.array(ages),
                                         'race_output': np.array(races),
...
# Print the DataFrame to inspect its content
print("Before dropping NaNs:")
print(df.head())

df = df.dropna()

# Print the DataFrame after dropping NaNs
...
# Define get_data_generator and CNN model here
...
for i, img_idx in enumerate(random_indices):
    ...
    ax.set_title('a:{}, g:{}, r:{}'.format(int(age_pred[img_idx]), ID_GENDER_MAP[gender_pred[img_idx]],
                                           ID_RACE_MAP[race_pred[img_idx]]))
    ax.set_xlabel('a:{}, g:{}, r:{}'.format(int(age_true[img_idx]), ID_GENDER_MAP[gender_true[img_idx]],
